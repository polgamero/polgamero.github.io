...
function injectStartingCoinStyles() {
  ...
  style.textContent = `
    ...
    .starting-coin-flight{position:relative;width:160px;height:160px;display:flex;align-items:center;justify-content:center;transform-style:preserve-3d;-webkit-transform-style:preserve-3d;will-change:transform;filter:drop-shadow(0 16px 20px rgba(0,0,0,.46));}
    ...
    .starting-coin-flight.has-landed{filter:drop-shadow(0 12px 18px rgba(0,0,0,.42)) drop-shadow(0 0 13px rgba(231,189,67,.22));}
...
export function showStartingCoinToss({ localName, rivalName, winnerSide = 'local' } = {}) {
  ...
  return new Promise(resolve => {
    ...
    setTimeout(() => {
      flight.classList.add('has-landed');
