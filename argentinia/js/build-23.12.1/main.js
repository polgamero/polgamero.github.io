...
const BUILD_REFRESH_SESSION_KEY = 'argentinia.build-refresh-attempt.v1';

function clearBuildRefreshAttempt() {
  try { sessionStorage.removeItem(BUILD_REFRESH_SESSION_KEY); } catch {}
}

function tryAutomaticBuildRefresh(serverVersion) {
  if (!serverVersion || typeof location === 'undefined') return false;
  const token = `${ENGINE_VERSION}->${serverVersion}`;
  try {
    if (sessionStorage.getItem(BUILD_REFRESH_SESSION_KEY) === token) return false;
    sessionStorage.setItem(BUILD_REFRESH_SESSION_KEY, token);
    const url = new URL(location.href);
    url.searchParams.set('build', String(serverVersion));
    url.searchParams.set('refresh', String(Date.now()));
    globalThis.__ARGENTINIA_BOOT_DIAG__?.mark?.('build_mismatch_auto_refresh', { loadedVersion: ENGINE_VERSION, serverVersion });
...
function renderBuildMismatchScreen(serverVersion) {
  const target = String(serverVersion || 'más nueva');
  const href = (() => {
    try {
      const url = new URL(location.href);
      url.searchParams.set('build', target);
      url.searchParams.set('refresh', String(Date.now()));
      return url.href;
    } catch { return location.href; }
  })();
  document.body.innerHTML = `<div style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:#08100b;color:#f0e0b0;font-family:system-ui;padding:24px"><div style="max-width:620px;border:2px solid #d4af37;border-radius:14px;padding:24px;text-align:center;background:#111a13"><h2>♻️ Actualización disponible</h2><p>Esta pestaña cargó Argentinia ${ENGINE_VERSION}, pero el servidor publica ${target}.</p><p>Argentinia intentó actualizarse automáticamente. Si seguís viendo esto, abrí la versión nueva con el botón.</p><p><a href="${href}" style="display:inline-block;background:#d4af37;color:#111;padding:12px 18px;border-radius:10px;font-weight:800;text-decoration:none">ABRIR VERSIÓN ${target}</a></p></div></div>`;
}
...
async function boot() {
  ...
  // 23.12.1: freshness ANTES de Firebase. Si quedó un HTML/build viejo, intentamos
  // una navegación cache-busteada una sola vez y evitamos cargar SDK/cartas innecesariamente.
  ...
  if (!freshness.ok) {
    ...
    if (tryAutomaticBuildRefresh(serverVersion)) return;
    renderBuildMismatchScreen(serverVersion);
    return;
  }
  clearBuildRefreshAttempt();
...
// 23.12.1 — IMPORTANTE: index.html carga main.js desde ./js/build-<VERSION>/.
// Todos los módulos internos de ESA carpeta que importan './main.js' resuelven la misma URL,
