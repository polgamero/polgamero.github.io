...
for(const label of ['Bajar Tierra','Entrada de Criatura','Entrada a Support','Entrada de Planeswalker','Pelear','Barajar biblioteca','Crear fichas','Transformar DFC','Animar Tierra','Castear Instantáneo / Conjuro']) assert.ok(director.includes(label));
...
assert.ok(version.includes("ENGINE_VERSION = '23.19.4.8'"));
assert.ok(workflow.includes('test_core_gameplay_feedback_expansion_23_19_4_7.mjs'),'CI whitelist retains v23.19.4.8 contract');
assert.ok(workflow.includes('Validate Core Gameplay Feedback Expansion 23.19.4.8'),'CI executes v23.19.4.8 contract');
