...
const fixtureDir=path.resolve(val('--fixtures',path.join(os.tmpdir(),'argentinia-headless-fixtures-23.18.1')));
...
async function runOne(seed,difficulty,identity){
  const args=[worker,'--seed',seed,'--difficulty',difficulty,'--identity',identity,'--maxTurns',String(maxTurns),'--maxSteps',String(maxSteps)];
...
async function runCase(c){
  ...
  if(!['completed'].includes(baseline.status)){
    ...
    fs.writeFileSync(fp,JSON.stringify(baseline,null,2)); fixtures.push(fp);
  }
...
const summary={version:'23.18.1',games,determinismRuns,concurrency,difficulties,identities,counts,totalActions,totalTurns,avgActions:Number((totalActions/games).toFixed(2)),avgTurns:Number((totalTurns/games).toFixed(2)),determinismFailures:determinismFailures.length,fixtures:fixtures.length,fixtureDir};
console.log(`HEADLESS_FULL_GAME_LAB_23_18_1_OK games=${games} deterministicRuns=${determinismRuns} completed=${counts.completed||0} coverageStops=${counts.coverage_stop||0} stalls=${counts.stall||0} invariants=${counts.invariant_failure||0} turnLimits=${counts.turn_limit||0} stepLimits=${counts.step_limit||0} determinismFailures=${determinismFailures.length} totalActions=${totalActions} totalTurns=${totalTurns}`);
