...
const end=main.indexOf("await returnToMainMenuAfterAbandon();",start);
assert.ok(start>=0 && end>start,'No se encontró el cleanup de abandono + retorno suave al menú');
const block=main.slice(start,end+"await returnToMainMenuAfterAbandon();".length);
assert.doesNotMatch(block,/location\.reload\(\)/,'Abandono normal no debe recargar: rompería fullscreen');
