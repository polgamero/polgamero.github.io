...
assert.ok(/^23\.19\.4\.(?:4|[5-9]|[1-9]\d+)$/.test(ENGINE_VERSION) || ENGINE_VERSION === '23.21.6',`23.19.4.4+ cumulative engine expected, got ${ENGINE_VERSION}`);
...
assert.ok(['DRAGONES EN BUENOS AIRES · Pool 900 + Dragon Archetype Foundation','Card Publication Control + Admin Card Identity + Browser Archetype UX','Basic Land Access + Admin Statistics Dashboard','Multiplayer Social Layer RC5.2 · Match Lifecycle + Mobile HUD Stabilization','Multiplayer Social Layer RC5.1 · Premium Halo + Mobile Geometry Stabilization','Multiplayer Social Layer RC5 · Social Delivery + Mobile HUD Hardening','Multiplayer Social Layer RC4 · Telemetry Recovery + PvP UX + Emote Studio','Multiplayer Social Layer · Chat + Emotes + Premium Cosmetics','ELO + Movimientos + Tournament Audit Fix + Bot Survival + Ranking/Market UX','Mercado de Pases · 1↔1 Server Authority + BUSCO Criteria','Torneo · 16-player Knockout Mode','Economy Write Firewall + Server-Required Cutover','Admin Economy + Statistics + Immutable Audit Authority','Match Settlement + Anti-Farming + Admission Control + Economy Pending UX','Store / Craft / Prebuilt / Classifieds / Username Authority + Packs / Cofre / Mythic + Deck Intelligence 2.0','Packs / Cofre / Mythic Authority + Deck Intelligence 2.0','Animation Tuning Matrix + Draggable Test Console','Animation Actor Parity + SFX Cue Semantics','Animation Actor Parity + SFX Cue Semantics + Admin Audio Targets','Rules Integrity + Combat UX + Bot Tactical Hotfix','Core Gameplay Feedback Expansion','Mass Event Cinematics + High-Impact Feedback','Commercial IP Remediation Wave 1 — Identity Clean-room','Commercial IP Remediation Wave 2 — Envase Hermético','Commercial IP Remediation Wave 3 — Venues + Institutions Genericization','Commercial IP Remediation — Residual YELLOW Closure','Global Terminology Clean-room — Owner Dictionary 44/44','Commercial Readiness Closure + Clean-room Verification','Semidiós Rules Text + Creencia UX Hotfix','Economy Authority Foundation + Secure Account Bootstrap','Economy Authority Foundation + Deck/Admin/Land UX Stabilization RC2'].includes(manifest.label));
const fastManifest=fs.readFileSync(path.join(root,'tools','ci_fast_contract_manifest_23_21_0.txt'),'utf8');
...
console.log(`pool=880 protocol=${ENGINE_PROTOCOL_VERSION} rules=23.13.80 unchanged`);
