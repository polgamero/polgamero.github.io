...
const maxSteps=Math.max(100, Number(args.maxSteps)||5000);
const maxTurns=Math.max(2, Number(args.maxTurns)||80);
...
const [{state,playCard,tapLocalLand,cancelPayment,checkGameOver},bot,combat,turn,stack,telemetry,rng,lab,keywords,replay] = await Promise.all([
...
function affordableSafeCreatureIndex(){
  const mana=untappedLocalLands().length;
  let best=-1,bestCmc=-1;
  state.localHand.forEach((c,i)=>{
    if(!lab.isHeadlessSafeCreature(c,identity) || state.activePlayer!=='local') return;
...
async function tryLocalCast(){
  const idx=affordableSafeCreatureIndex();
  ...
  while(state.pendingCastTransaction && state.pendingCost && taps<state.localLands.length+2){
    const land=untappedLocalLands()[0];
    if(!land) break;
...
async function localStep(){
  ...
  if(state.activePlayer==='local' && (state.phase==='main1'||state.phase==='main2')){
    ...
    if(await tryLocalCast()){ pushAction('LOCAL_CAST_SAFE_CREATURE'); return; }
  }
...
const localBuild=lab.buildHeadlessSafeDeck({creatures:pool['criaturas.json'],lands:pool['tierras.json'],identity,seed,side:'local'});
const rivalBuild=lab.buildHeadlessSafeDeck({creatures:pool['criaturas.json'],lands:pool['tierras.json'],identity,seed,side:'rival'});
...
pushAction('GAME_INITIALIZED',{identity,localSafePool:localBuild.safeCreatureCount,rivalSafePool:rivalBuild.safeCreatureCount});
...
const result=lab.summarizeHeadlessRun({seed,difficulty,steps:actions.length,snapshot:finalSnapshot,actions,unsupported:reason?.startsWith('UNSUPPORTED_PENDING:')?[reason.split(':')[1]]:[],invariantFindings,status,reason,traceHash});
...
if(args.debug) { result.debugSnapshot=finalSnapshot; result.debugActions=actions; }
