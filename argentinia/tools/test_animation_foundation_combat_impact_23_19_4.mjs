...
need(ENGINE_VERSION === '23.21.6', `engine=${ENGINE_VERSION}`);
need(ENGINE_PROTOCOL_VERSION === 'mp-23.19.2', `protocol=${ENGINE_PROTOCOL_VERSION}`);
need(FIRESTORE_RULES_VERSION === '23.13.86', `rules=${FIRESTORE_RULES_VERSION}`);
need(POOL_BASELINE.total === 900, `pool=${POOL_BASELINE.total}`);
...
need(manifest.engineVersion === '23.21.6' && manifest.firestoreRulesVersion === '23.13.86' && manifest.pool === 900, 'build-manifest');
...
console.log('pool=880 protocol=mp-23.19.2 rules=23.13.80');
