...
assert.ok(version.includes("ENGINE_VERSION = '23.18'") || (()=>{ try { if (!version.includes("ENGINE_VERSION = '23.18'")) return /ENGINE_VERSION = '(?:23\.14\.(?:[5-9]|[1-9][0-9]+)|23\.(?:1[5-9]|[2-9][0-9])\.\d+(?:\.\d+)?|(?:2[4-9]|[3-9]\d)\.\d+\.\d+)'|ENGINE_VERSION = '23\.16\.1(?:\.1)?'|ENGINE_VERSION = '23\.16\.(?:2(?:\.1)?|3(?:\.1)?|4(?:\.1)?|5(?:\.[12])?)'/.test(version); } catch { return false; } })());
