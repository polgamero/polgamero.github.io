import { HttpsError } from 'firebase-functions/v2/https';

const MAP = Object.freeze({
  AUTH_REQUIRED: ['unauthenticated', 'Tenés que iniciar sesión.'],
  INVALID_OPERATION_ID: ['invalid-argument', 'operationId inválido.'],
  OPERATION_ID_PAYLOAD_MISMATCH: ['failed-precondition', 'Ese operationId ya fue usado con otro payload.'],
  ECONOMY_DISABLED: ['unavailable', 'La economía está temporalmente pausada.'],
  ECONOMY_CLIENT_TOO_OLD: ['failed-precondition', 'Actualizá Argentinia para usar esta operación económica.'],
  INVALID_ECONOMY_REQUEST: ['invalid-argument', 'Solicitud económica inválida.'],
  RATE_LIMITED: ['resource-exhausted', 'Demasiadas operaciones económicas. Esperá un momento e intentá de nuevo.'],
  USERNAME_TAKEN: ['already-exists', 'Ese nombre ya está usado.'],
  USERNAME_ALREADY_CONFIGURED: ['failed-precondition', 'La cuenta ya tiene un nombre configurado.'],
  USERNAME_REQUIRED: ['failed-precondition', 'Primero elegí tu nombre en Argentinia.'],
  USERNAME_LENGTH: ['invalid-argument', 'Usá entre 3 y 18 caracteres.'],
  USERNAME_CHARS: ['invalid-argument', 'Usá letras, números, espacios o guion bajo.'],
  USERNAME_KEY_LENGTH: ['invalid-argument', 'El nombre debe tener al menos 3 caracteres visibles.'],
  USERNAME_RESERVED: ['invalid-argument', 'Ese nombre está reservado por Argentinia.'],
  USERNAME_BLOCKED: ['invalid-argument', 'Ese nombre no está permitido.'],
  STARTER_IDENTITY_INVALID: ['invalid-argument', 'Elegí uno o dos colores válidos para el mazo inicial.'],
  STARTER_ALREADY_COMPLETED: ['failed-precondition', 'El mazo inicial ya fue asignado.'],
  STARTER_PROFILE_MISSING: ['failed-precondition', 'La cuenta todavía no está inicializada.'],
  STARTER_POOL_INVALID: ['internal', 'El pool trusted del servidor no pudo construir un starter válido.'],
  PROFILE_MISSING: ['failed-precondition', 'No se encontró tu perfil.'],
  CAMPAIGN_POLICY_UNAVAILABLE: ['unavailable', 'No se pudo validar la política de campañas. Probá de nuevo.'],
  STORE_INSUFFICIENT_POINTS: ['failed-precondition', 'No te alcanzan los puntos para este sobre.'],
  CRAFT_CARD_NOT_ELIGIBLE: ['invalid-argument', 'Por ahora sólo se pueden mejorar Criaturas.'],
  CRAFT_KEYWORD_INVALID: ['invalid-argument', 'Esa mejora no está habilitada.'],
  CRAFT_KEYWORD_ALREADY_PRESENT: ['failed-precondition', 'Esa criatura ya tiene esa habilidad de forma natural. Elegí otra mejora.'],
  CRAFT_CARD_NOT_OWNED: ['failed-precondition', 'No tenés esa carta en tu colección.'],
  CRAFT_ALREADY_ENHANCED: ['failed-precondition', 'Esa carta ya tiene una mejora.'],
  CRAFT_INSUFFICIENT_FICHAS: ['failed-precondition', 'No te alcanzan las Fichas.'],
  CRAFT_DECK_ENHANCED_LIMIT_CONFLICT: ['failed-precondition', 'Uno de tus mazos usa todas las copias normales de esta carta y ya alcanzó el máximo de cartas mejoradas. Editá ese mazo antes de mejorarla.'],
  CRAFT_EVOLUTION_COPY_CONFLICT: ['failed-precondition', 'Esa carta ya tiene una copia evolucionada. Necesitás otra copia física para aplicar además una mejora.'],
  WORKSHOP_DISABLED: ['unavailable', 'El Taller está temporalmente deshabilitado.'],
  WORKSHOP_MACHINE_INVALID: ['invalid-argument', 'Esa máquina del Taller no existe.'],
  WORKSHOP_MACHINE_UNAVAILABLE: ['failed-precondition', 'Esa máquina todavía no está disponible para jugadores.'],
  WORKSHOP_ALREADY_UNLOCKED: ['failed-precondition', 'Esa máquina ya está desbloqueada.'],
  WORKSHOP_INSUFFICIENT_FUNDS: ['failed-precondition', 'No te alcanzan los Puntos o las Fichas para desbloquear esta máquina.'],
  CRAFT_WORKSHOP_LOCKED: ['failed-precondition', 'Desbloqueá el Generador de mejoras en Mi Taller antes de fabricar una mejora.'],
  ACHIEVEMENTS_DISABLED: ['unavailable', 'Los Logros están temporalmente deshabilitados.'],
  ACHIEVEMENT_INVALID: ['invalid-argument', 'Ese logro no existe.'],
  ACHIEVEMENT_DISABLED: ['failed-precondition', 'Ese logro está deshabilitado.'],
  ACHIEVEMENT_NOT_REACHED: ['failed-precondition', 'Todavía no alcanzaste ese logro.'],
  ACHIEVEMENT_ALREADY_CLAIMED: ['failed-precondition', 'Ese logro ya fue reclamado.'],
  ESSENCE_CONVERSION_DISABLED: ['unavailable', 'La conversión a Esencia está temporalmente deshabilitada.'],
  ESSENCE_CONVERSION_INVALID: ['invalid-argument', 'La cantidad de Esencia solicitada no es válida.'],
  ESSENCE_INSUFFICIENT_FUNDS: ['failed-precondition', 'No te alcanzan los Puntos o las Fichas para generar esa Esencia.'],
  EVOLUTION_WORKSHOP_LOCKED: ['failed-precondition', 'Desbloqueá la Cápsula de evolución en Mi Taller antes de evolucionar cartas.'],
  EVOLUTION_CARD_NOT_ELIGIBLE: ['invalid-argument', 'Esa carta no tiene una ruta de evolución habilitada.'],
  EVOLUTION_MAX_STAGE: ['failed-precondition', 'Esa carta ya alcanzó su evolución máxima.'],
  EVOLUTION_COPIES_REQUIRED: ['failed-precondition', 'No tenés las copias requeridas de esa carta para esta evolución.'],
  EVOLUTION_COPY_CONFLICT: ['failed-precondition', 'No hay suficientes copias físicas para sostener las variantes especiales de esta carta.'],
  EVOLUTION_INSUFFICIENT_FUNDS: ['failed-precondition', 'No te alcanzan los Puntos, Fichas o Esencia para esta evolución.'],
  EVOLUTION_DECK_LIMIT_CONFLICT: ['failed-precondition', 'Uno de tus mazos necesita usar la copia evolucionada pero ya alcanzó el máximo de evolucionadas. Editá ese mazo antes de continuar.'],
  INDUSTRIAL_MIX_WORKSHOP_LOCKED: ['failed-precondition', 'Desbloqueá la Mezcladora industrial en Mi Taller antes de mezclar cartas.'],
  INDUSTRIAL_MIX_CARD_INVALID: ['invalid-argument', 'Esa carta no pertenece al pool válido de Argentinia.'],
  INDUSTRIAL_MIX_MAX_RARITY: ['failed-precondition', 'Las cartas Mythic no pueden mezclarse porque no existe una rareza superior contigua.'],
  INDUSTRIAL_MIX_COPIES_REQUIRED: ['failed-precondition', 'Necesitás 3 copias realmente libres de esa carta para usar la Mezcladora.'],
  INDUSTRIAL_MIX_INSUFFICIENT_FUNDS: ['failed-precondition', 'No te alcanzan los Puntos, Fichas o Esencia para esta mezcla.'],
  INDUSTRIAL_MIX_POOL_EMPTY: ['unavailable', 'No hay cartas publicadas disponibles en la rareza de destino. Probá nuevamente más adelante.'],
  PREBUILT_NOT_FOUND: ['not-found', 'Ese mazo prearmado no existe.'],
  PREBUILT_NAME_REQUIRED: ['invalid-argument', 'Elegí un nombre para el mazo.'],
  PREBUILT_NAME_TOO_LONG: ['invalid-argument', 'El nombre del mazo no puede tener más de 30 caracteres.'],
  PREBUILT_DECK_LIMIT: ['failed-precondition', 'Ya alcanzaste el máximo de mazos guardados.'],
  PREBUILT_ALREADY_PURCHASED: ['failed-precondition', 'Ese mazo prearmado ya fue comprado por esta cuenta.'],
  PREBUILT_INSUFFICIENT_FUNDS: ['failed-precondition', 'No te alcanzan los puntos o las Fichas.'],
  CLASSIFIEDS_WEEK_NOT_PUBLISHED: ['failed-precondition', 'Los Avisos Clasificados de esta semana todavía no fueron publicados.'],
  CLASSIFIEDS_SCHEDULE_INVALID: ['internal', 'La publicación de Avisos Clasificados no supera su contrato trusted.'],
  CLASSIFIEDS_CARD_NOT_OFFERED: ['failed-precondition', 'Esa carta no forma parte de los Avisos Clasificados de esta semana.'],
  CLASSIFIEDS_ALREADY_PURCHASED: ['failed-precondition', 'Esa carta ya la compraste esta semana.'],
  CLASSIFIEDS_INSUFFICIENT_FUNDS: ['failed-precondition', 'No te alcanzan los puntos o las Fichas para comprar esta carta.'],
  CLASSIFIEDS_SLOT_LIMIT_REACHED: ['failed-precondition', 'Ya alcanzaste el cupo semanal para esa rareza.'],
  USERNAME_ACTIVE_MATCH: ['failed-precondition', 'Terminá tu partida multiplayer antes de cambiar el nombre.'],
  USERNAME_SAME: ['failed-precondition', 'Ese ya es tu nombre actual.'],
  USERNAME_NOT_ENOUGH_FICHAS: ['failed-precondition', 'No tenés suficientes Fichas.'],
  USERNAME_REGISTRY_MISMATCH: ['failed-precondition', 'La reserva del nombre actual está inconsistente.'],
  ADMIN_REQUIRED: ['permission-denied', 'Esta operación es exclusiva del admin.'],
  REGISTRATION_PAUSED: ['resource-exhausted', 'El registro de nuevos jugadores está temporalmente pausado.'],
  REGISTRATION_CAPACITY_REACHED: ['resource-exhausted', 'Argentinia alcanzó temporalmente su cupo de nuevos jugadores.'],
  REGISTRATION_DAILY_LIMIT_REACHED: ['resource-exhausted', 'Se alcanzó el cupo de altas de hoy. Probá nuevamente más adelante.'],
  REGISTRATION_UNAVAILABLE: ['unavailable', 'No se pudo validar el cupo de nuevos jugadores. Probá de nuevo.'],
  MATCH_REWARD_INVALID: ['invalid-argument', 'Solicitud de premio de partida inválida.'],
  MATCH_REWARD_DIFFICULTY_REQUIRED: ['invalid-argument', 'Falta la dificultad de la partida contra el Tano.'],
  MATCH_REWARD_MATCH_REQUIRED: ['invalid-argument', 'Falta el identificador de la partida multiplayer.'],
  PVP_MATCH_NOT_FOUND: ['not-found', 'No se encontró la partida multiplayer.'],
  PVP_NOT_MATCH_PARTICIPANT: ['permission-denied', 'Tu cuenta no participa de esa partida.'],
  PVP_MATCH_NOT_TERMINAL: ['failed-precondition', 'La partida todavía no tiene evidencia terminal válida.'],
  PVP_MATCH_NOT_READY: ['failed-precondition', 'La partida no llegó al estado ready requerido para liquidar premios.'],
  PVP_MATCH_NOT_SEALED: ['failed-precondition', 'La partida todavía no fue sellada por el servidor.'],
  PVP_TERMINAL_EVIDENCE_MISMATCH: ['failed-precondition', 'La evidencia terminal de la partida es inconsistente.'],
  PVP_OUTCOME_MISMATCH: ['failed-precondition', 'El resultado solicitado no coincide con la evidencia del servidor.'],
  PVP_ABANDON_EVIDENCE_MISMATCH: ['failed-precondition', 'La penalidad de abandono no coincide con la evidencia del servidor.'],
  ABANDON_MODE_INVALID: ['invalid-argument', 'Modo de abandono inválido.'],
  ABANDON_RECEIPT_REQUIRED: ['invalid-argument', 'Falta el identificador durable del abandono.'],
  ABANDON_RECEIPT_CONFLICT: ['failed-precondition', 'La partida ya tiene una liquidación incompatible con un abandono.'],
  ADMIN_REWARD_REPAIR_ABANDONED: ['failed-precondition', 'Una partida abandonada no puede repararse como premio positivo.'],
  DAILY_REWARD_INVALID: ['invalid-argument', 'Ese premio diario no existe.'],
  DAILY_LOGIN_REQUIRED: ['failed-precondition', 'Volvé a entrar para sincronizar tu recompensa diaria con el servidor.'],
  DAILY_REWARD_NOT_AVAILABLE: ['failed-precondition', 'Ese premio no está disponible o ya fue reclamado.'],
  DAILY_DEBUG_MODE_INVALID: ['invalid-argument', 'Modo de debug diario inválido.'],
  DAILY_DEBUG_MAX: ['failed-precondition', 'El reloj QA ya está en el máximo de +30 días.'],
  TOURNAMENT_NOT_FOUND: ['not-found', 'No se encontró el torneo.'],
  TOURNAMENT_NOT_ACTIVE: ['failed-precondition', 'Ese torneo ya terminó.'],
  TOURNAMENT_MATCH_NOT_ACTIVE: ['failed-precondition', 'Esa partida de torneo no está activa.'],
  TOURNAMENT_BRACKET_INVALID: ['internal', 'El fixture del torneo es inconsistente.'],
  TOURNAMENT_RECEIPT_CONFLICT: ['failed-precondition', 'La partida de torneo ya tiene un resultado incompatible.'],
  TRADE_LISTING_EXISTS: ['failed-precondition', 'Ya tenés una publicación activa en el Mercado de Pases.'],
  TRADE_LISTING_LIMIT: ['resource-exhausted', 'Ya alcanzaste el máximo configurado de publicaciones activas.'],
  TRADE_LISTING_NOT_FOUND: ['not-found', 'No se encontró esa publicación activa.'],
  TRADE_LISTING_NOT_ACTIVE: ['failed-precondition', 'La publicación ya no está activa.'],
  TRADE_CARD_INVALID: ['invalid-argument', 'Esa carta no pertenece al pool válido de Argentinia.'],
  CARD_DISABLED: ['failed-precondition', 'Esa carta está temporalmente deshabilitada y no puede usarse en nuevas operaciones.'],
  TRADE_CARD_NOT_TRADABLE: ['failed-precondition', 'No tenés una copia libre de esa carta para intercambiar.'],
  TRADE_CRITERIA_INVALID: ['invalid-argument', 'Configurá una cantidad válida de criterios BUSCO según el límite actual, o elegí Acepto cualquier carta.'],
  TRADE_OFFER_LIMIT: ['resource-exhausted', 'Esta publicación ya alcanzó el máximo configurado de ofertas activas.'],
  TRADE_OUTGOING_LIMIT: ['resource-exhausted', 'Ya alcanzaste el máximo configurado de ofertas salientes activas.'],
  TRADE_OFFER_EXISTS: ['failed-precondition', 'Ya hiciste una oferta activa para esta publicación.'],
  TRADE_OFFER_NOT_FOUND: ['not-found', 'No se encontró esa oferta activa.'],
  TRADE_OFFER_NOT_MATCHING: ['failed-precondition', 'La carta ofrecida no cumple ninguno de los criterios BUSCO.'],
  TRADE_WEEKLY_LIMIT: ['resource-exhausted', 'Uno de los jugadores ya alcanzó el máximo semanal configurado de intercambios completados.'],
  TRADE_SELF_OFFER: ['failed-precondition', 'No podés hacer una oferta sobre tu propia publicación.'],
  TRADE_RESERVATION_CONFLICT: ['failed-precondition', 'La reserva ya no puede sostenerse con la colección y los mazos actuales.'],
  EMOTE_CATALOG_INVALID: ['invalid-argument', 'El catálogo de emojis no es válido.'],
  EMOTE_CATALOG_SIZE_INVALID: ['invalid-argument', 'El catálogo debe tener entre 1 y 128 emojis.'],
  EMOTE_CATALOG_ROW_INVALID: ['invalid-argument', 'Hay una fila de emoji inválida.'],
  EMOTE_CATALOG_ID_INVALID: ['invalid-argument', 'Hay un ID de emoji inválido.'],
  EMOTE_CATALOG_DUPLICATE_ID: ['invalid-argument', 'Hay IDs de emoji duplicados.'],
  EMOTE_CATALOG_LABEL_INVALID: ['invalid-argument', 'Todos los emojis necesitan un nombre.'],
  EMOTE_CATALOG_ASSET_INVALID: ['invalid-argument', 'Imagen/audio debe ser un nombre de asset local seguro.'],
  EMOTE_CATALOG_ANIMATION_INVALID: ['invalid-argument', 'La animación elegida no es válida.'],
  EMOTE_CATALOG_PRICE_INVALID: ['invalid-argument', 'Un emoji premium debe costar entre 1 y 100000 Puntos.'],
  EMOTE_CATALOG_FIELD_INVALID: ['invalid-argument', 'El catálogo contiene un campo no permitido.'],
  EMOTE_NOT_FOUND: ['not-found', 'Ese emoji no existe en el catálogo actual.'],
  EMOTE_FREE_INCLUDED: ['failed-precondition', 'Ese emoji ya está incluido para todos los jugadores.'],
  EMOTE_INACTIVE: ['failed-precondition', 'Ese emoji está desactivado por el administrador.'],
  EMOTE_ALREADY_OWNED: ['failed-precondition', 'Ya tenés ese emoji.'],
  EMOTE_INSUFFICIENT_POINTS: ['failed-precondition', 'No te alcanzan los puntos para comprar ese emoji.'],
  MULTIPLAYER_SOCIAL_MATCH_INVALID: ['invalid-argument', 'La partida multiplayer no es válida para comunicación.'],
  MULTIPLAYER_SOCIAL_TYPE_INVALID: ['invalid-argument', 'Ese tipo de comunicación no es válido.'],
  MULTIPLAYER_SOCIAL_MATCH_CLOSED: ['failed-precondition', 'El chat ya no está disponible porque la partida terminó.'],
  MULTIPLAYER_SOCIAL_NOT_READY: ['failed-precondition', 'El chat se habilita cuando ambos jugadores están listos.'],
  MULTIPLAYER_CHAT_INVALID: ['invalid-argument', 'El mensaje está vacío o supera el máximo permitido.'],
  MULTIPLAYER_CHAT_RATE_LIMIT: ['resource-exhausted', 'Estás enviando mensajes demasiado rápido.'],
  MULTIPLAYER_EMOTE_INVALID: ['invalid-argument', 'Ese emoji no existe.'],
  MULTIPLAYER_EMOTE_NOT_OWNED: ['permission-denied', 'Ese emoji premium no pertenece a tu cuenta.'],
  MULTIPLAYER_EMOTE_RATE_LIMIT: ['resource-exhausted', 'Esperá un momento antes de mandar otro emoji.'],
  LOBBY_CHAT_TYPE_INVALID: ['invalid-argument', 'Ese tipo de mensaje no está habilitado en el Lobby.'],
  LOBBY_CHAT_RATE_LIMIT: ['resource-exhausted', 'Estás enviando mensajes demasiado rápido en el Lobby.'],
  LOBBY_CHAT_DUPLICATE: ['failed-precondition', 'Evitá repetir el mismo mensaje seguido.'],
  LOBBY_CHAT_PROFANITY: ['invalid-argument', 'Ese mensaje contiene lenguaje no permitido.'],
  LOBBY_CHAT_MESSAGE_NOT_FOUND: ['not-found', 'Ese mensaje ya no existe en el Lobby.'],
  MULTIPLAYER_SOCIAL_SCOPE_INVALID: ['invalid-argument', 'Ese canal social no existe.'],
  MULTIPLAYER_CHALLENGE_TARGET_INVALID: ['invalid-argument', 'El jugador invitado no es válido.'],
  MULTIPLAYER_CHALLENGE_SESSION_INVALID: ['invalid-argument', 'La sesión multiplayer no es válida para este desafío.'],
  MULTIPLAYER_CHALLENGE_INVALID: ['invalid-argument', 'La invitación multiplayer no es válida.'],
  MULTIPLAYER_CHALLENGE_SELF: ['invalid-argument', 'No podés invitarte a vos mismo.'],
  MULTIPLAYER_CHALLENGE_INVITER_UNAVAILABLE: ['failed-precondition', 'Ya no estás disponible en el Lobby.'],
  MULTIPLAYER_CHALLENGE_TARGET_UNAVAILABLE: ['failed-precondition', 'Ese jugador ya no está disponible en el Lobby.'],
  MULTIPLAYER_CHALLENGE_ALREADY_PENDING: ['failed-precondition', 'Ya tenés una invitación pendiente.'],
  MULTIPLAYER_CHALLENGE_TARGET_BUSY: ['failed-precondition', 'Ese jugador ya tiene otra invitación pendiente.'],
  MULTIPLAYER_CHALLENGE_ACTIVE_MATCH: ['failed-precondition', 'Uno de los jugadores ya tiene una partida multiplayer activa.'],
  MULTIPLAYER_CHALLENGE_RATE_LIMIT: ['resource-exhausted', 'Estás enviando invitaciones demasiado rápido.'],
  MULTIPLAYER_CHALLENGE_PAIR_COOLDOWN: ['resource-exhausted', 'Esperá unos segundos antes de volver a invitar al mismo jugador.'],
  MULTIPLAYER_CHALLENGE_NOT_FOUND: ['not-found', 'La invitación ya no existe.'],
  MULTIPLAYER_CHALLENGE_NOT_PARTICIPANT: ['permission-denied', 'Tu cuenta no participa de esa invitación.'],
  MULTIPLAYER_CHALLENGE_ALREADY_RESOLVED: ['failed-precondition', 'La invitación ya fue resuelta.'],
  MULTIPLAYER_CHALLENGE_EXPIRED: ['deadline-exceeded', 'La invitación expiró.'],
  MULTIPLAYER_CHALLENGE_INVITEE_ONLY: ['permission-denied', 'Sólo el jugador invitado puede responder esta invitación.'],
  MULTIPLAYER_CHALLENGE_INVITER_ONLY: ['permission-denied', 'Sólo quien envió la invitación puede cancelarla.'],
  MULTIPLAYER_CHALLENGE_NOT_EXPIRED: ['failed-precondition', 'La invitación todavía no expiró.'],
  MULTIPLAYER_CHALLENGE_ACTION_INVALID: ['invalid-argument', 'Acción de invitación multiplayer inválida.'],
  MULTIPLAYER_CHALLENGE_LOCK_LOST: ['failed-precondition', 'La invitación perdió su reserva; volvé a intentarlo.'],
  MULTIPLAYER_CHALLENGE_MATCH_CODE_EXHAUSTED: ['resource-exhausted', 'No se pudo reservar un código de partida. Probá otra vez.'],

  PUBLIC_PROFILE_NOT_FOUND: ['not-found', 'El perfil público no está disponible.'],
  PUBLIC_PROFILE_TARGET_INVALID: ['invalid-argument', 'El jugador indicado no es válido.'],
  PUBLIC_PROFILE_FAVORITE_INVALID: ['invalid-argument', 'Esa carta no pertenece al pool válido de Argentinia.'],
  PUBLIC_PROFILE_FAVORITE_NOT_OWNED: ['failed-precondition', 'Sólo podés elegir como favorita una carta que poseas actualmente.'],
  COMMUNITY_BANNED: ['permission-denied', 'Tu cuenta tiene una restricción activa de Moderación.'],
  COMMUNITY_ACTION_INVALID: ['invalid-argument', 'Acción de Moderación inválida.'],
  COMMUNITY_TARGET_INVALID: ['invalid-argument', 'El usuario indicado no es válido.'],
  COMMUNITY_TEXT_INVALID: ['invalid-argument', 'El texto de Moderación no es válido.'],
  COMMUNITY_BAN_DURATION_INVALID: ['invalid-argument', 'La duración del ban no es válida.'],
  COMMUNITY_BAN_REASON_INVALID: ['invalid-argument', 'Escribí un motivo válido para el ban.'],
  COMMUNITY_CASE_KIND_INVALID: ['invalid-argument', 'Ese tipo de caso de Moderación no es válido.'],
  COMMUNITY_CASE_SUBJECT_INVALID: ['invalid-argument', 'El asunto del caso no es válido.'],
  COMMUNITY_CASE_TEXT_INVALID: ['invalid-argument', 'El mensaje a Moderación está vacío o es demasiado largo.'],
  COMMUNITY_CASE_SELF_REPORT: ['failed-precondition', 'No podés reportarte a vos mismo.'],
  COMMUNITY_MESSAGE_NOT_FOUND: ['not-found', 'Ese mensaje ya no está disponible para reportar.'],
  COMMUNITY_CASE_INVALID: ['invalid-argument', 'El caso de Moderación no es válido.'],
  COMMUNITY_CASE_NOT_FOUND: ['not-found', 'No se encontró ese caso de Moderación.'],
  COMMUNITY_CASE_NOT_RESOLVED: ['failed-precondition', 'Ese caso todavía no fue resuelto.'],
  COMMUNITY_CASE_RESPONSE_INVALID: ['invalid-argument', 'La respuesta de Moderación está vacía o es demasiado larga.'],
  USERNAME_TRADE_RESERVED: ['failed-precondition', 'Cancelá tu publicación y tus ofertas del Mercado de Pases antes de cambiar el nombre.'],
  INTERNAL: ['internal', 'Error interno de Economy Authority.']
});

export function economyError(code, details = undefined, overrideMessage = undefined) {
  const [httpsCode, defaultMessage] = MAP[code] || MAP.INTERNAL;
  const error = new HttpsError(httpsCode, overrideMessage || defaultMessage, { code, ...(details || {}) });
  error.argentiniaCode = code;
  return error;
}

export function errorCode(error) {
  return error?.argentiniaCode || error?.details?.code || error?.code || 'INTERNAL';
}
